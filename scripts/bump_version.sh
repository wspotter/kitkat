...
do
    case "${opt}" in
        p)
            ...
            cd ../emacs
            sed -E -i.bak "s/^;; Version: (.*)/;; Version: $current_version/" KIT.el
            git add KIT.el
            rm *.bak
            ...
            git add \
                ...
                $project_root/src/interface/emacs/KIT.el \
                $project_root/manifest.json \
                $project_root/versions.json
            git commit -m "Release KIT version $current_version"
        ...
        n)
            ...
            sed -E -i.bak "s/^;; Version: (.*)/;; Version: $next_version/" KIT.el
            rm *.bak
            ...
            git add \
                ...
                $project_root/src/interface/obsidian/versions.json \
                $project_root/src/interface/emacs/KIT.el
            git commit -m "Bump KIT to pre-release version $next_version"
            ;;
        ?)
