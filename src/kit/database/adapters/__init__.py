...
def require_valid_user(func: Callable[P, T]) -> Callable[P, T]:
    ...
    def sync_wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        ...
        user = next((arg for arg in args if isinstance(arg, KITUser)), None)
        if not user:
            user = next((val for val in kwargs.values() if isinstance(val, KITUser)), None)

        # Throw error if user is not found
        if not user:
            raise ValueError("KIT user argument required but not provided.")
...
def arequire_valid_user(func: Callable[P, Coroutine[Any, Any, T]]) -> Callable[P, Coroutine[Any, Any, T]]:
    ...
    async def async_wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        ...
        user = next((arg for arg in args if isinstance(arg, KITUser)), None)
        if not user:
            user = next((v for v in kwargs.values() if isinstance(v, KITUser)), None)
...
@arequire_valid_user
async def set_notion_config(token: str, user: KITUser):
...
@require_valid_user
def create_KIT_token(user: KITUser, name=None):
    "Create KIT API key for user"
    token = f"kk-{secrets.token_urlsafe(32)}"
    name = name or f"{generate_random_name().title()}"
    return KITApiUser.objects.create(token=token, user=user, name=name)


@arequire_valid_user
async def acreate_KIT_token(user: KITUser, name=None):
    "Create KIT API key for user"
    token = f"kk-{secrets.token_urlsafe(32)}"
    name = name or f"{generate_random_name().title()}"
    return await KITApiUser.objects.acreate(token=token, user=user, name=name)


@require_valid_user
def get_KIT_tokens(user: KITUser):
    "Get all KIT API keys for user"
    return list(KITApiUser.objects.filter(user=user))


@arequire_valid_user
async def delete_KIT_token(user: KITUser, token: str):
    "Delete KIT API Key for user"
    await KITApiUser.objects.filter(token=token, user=user).adelete()


async def get_or_create_user(token: dict) -> KITUser:
...
async def aget_or_create_user_by_phone_number(phone_number: str) -> tuple[KITUser, bool]:
    is_new = False
...
@arequire_valid_user
async def aset_user_phone_number(user: KITUser, phone_number: str) -> KITUser:
    if is_none_or_empty(phone_number):
        return None
...
@arequire_valid_user
async def aremove_phone_number(user: KITUser) -> KITUser:
...
async def acreate_user_by_phone_number(phone_number: str) -> KITUser:
    if is_none_or_empty(phone_number):
        return None
    user, _ = await KITUser.objects.filter(phone_number=phone_number).aupdate_or_create(
...
async def aget_or_create_user_by_email(input_email: str, check_deliverability=False) -> tuple[KITUser, bool]:
    ...
    is_existing_user = await KITUser.objects.filter(email=email).aexists()
    ...
    user, is_new = await KITUser.objects.filter(email=email).aupdate_or_create(
...
async def aget_user_validated_by_email_verification_code(code: str, email: str) -> tuple[Optional[KITUser], bool]:
    # Normalize the email address
    normalized_email, _ = normalize_email(email)

    # Check if verification code exists for the user
    user = await KITUser.objects.filter(email_verification_code=code, email=normalized_email).afirst()
    if not user:
        return None, False
...
async def create_user_by_google_token(token: dict) -> KITUser:
    user, _ = await KITUser.objects.filter(email=token.get("email")).aupdate_or_create(
...
@require_valid_user
def set_user_name(user: KITUser, first_name: str, last_name: str) -> KITUser:
...
@require_valid_user
def get_user_name(user: KITUser):
...
@require_valid_user
def get_user_photo(user: KITUser):
    google_profile: GoogleUser = GoogleUser.objects.filter(user=user).first()
    if google_profile:
...
@arequire_valid_user
async def aget_user_subscription_state(user: KITUser) -> str:
    """Get subscription state of user
    Valid state transitions: trial -> subscribed <-> unsubscribed OR expired
    """
    user_subscription = await Subscription.objects.filter(user=user).afirst()
...
@arequire_valid_user
async def ais_user_subscribed(user: KITUser) -> bool:
    """
...
@require_valid_user
def is_user_subscribed(user: KITUser) -> bool:
    """
    Get whether the user is subscribed
    """
    if not state.billing_enabled or state.anonymous_mode:
...
async def aget_user_by_email(email: str) -> KITUser:
    return await KITUser.objects.filter(email=email).afirst()


def get_user_by_email(email: str) -> KITUser:
    return KITUser.objects.filter(email=email).first()


async def aget_user_by_uuid(uuid: str) -> KITUser:
    return await KITUser.objects.filter(uuid=uuid).afirst()


async def get_user_by_token(token: dict) -> KITUser:
...
async def aget_user_by_phone_number(phone_number: str) -> KITUser:
    if is_none_or_empty(phone_number):
        return None
    matched_user = await KITUser.objects.filter(phone_number=phone_number).prefetch_related("subscription").afirst()

    if not matched_user:
        return None

    # If the user with this phone number does not have an email account with KIT, return the user
    if is_none_or_empty(matched_user.email):
        return matched_user

    # If the user has an email account with KIT and a verified number, return the user
...
async def retrieve_user(session_id: str) -> KITUser:
    ...
    user = await KITUser.objects.filter(id=session_data.get("_auth_user_id")).afirst()
    if not user:
        raise HTTPException(status_code=401, detail="Invalid user")
    return user


def get_all_users() -> BaseManager[KITUser]:
    return KITUser.objects.all()


@require_valid_user
def get_user_github_config(user: KITUser):
    config = GithubConfig.objects.filter(user=user).prefetch_related("githubrepoconfig").first()
    return config


@require_valid_user
def get_user_notion_config(user: KITUser):
...
@arequire_valid_user
async def aget_user_name(user: KITUser):
    full_name = user.get_full_name()
    if not is_none_or_empty(full_name):
        return full_name
...
@arequire_valid_user
async def set_user_github_config(user: KITUser, pat_token: str, repos: list):
...
class AgentAdapters:
    DEFAULT_AGENT_NAME = "KIT"
    DEFAULT_AGENT_SLUG = "KIT"

    @staticmethod
    async def aget_readonly_agent_by_slug(agent_slug: str, user: KITUser):
        return (
    ...
    @staticmethod
    @arequire_valid_user
    async def adelete_agent_by_slug(agent_slug: str, user: KITUser):
    ...
    @staticmethod
    async def aget_agent_by_slug(agent_slug: str, user: KITUser):
        return (
            await Agent.objects.filter(
    ...
    @staticmethod
    async def aget_agent_by_name(agent_name: str, user: KITUser):
        return (
            await Agent.objects.filter(
    ...
    @staticmethod
    def get_agent_by_slug(slug: str, user: KITUser = None):
        if user:
    ...
    @staticmethod
    def get_all_accessible_agents(user: KITUser = None):
    ...
    @staticmethod
    async def aget_all_accessible_agents(user: KITUser = None) -> List[Agent]:
        agents = await sync_to_async(AgentAdapters.get_all_accessible_agents)(user)
        return await sync_to_async(list)(agents)

    @staticmethod
    async def ais_agent_accessible(agent: Agent, user: KITUser) -> bool:
    ...
    @staticmethod
    def get_agent_chat_model(agent: Agent, user: Optional[KITUser]) -> Optional[ChatModel]:
        """
    ...
    @staticmethod
    async def aget_agent_chat_model(agent: Agent, user: Optional[KITUser]) -> Optional[ChatModel]:
...
class PublicConversationAdapters:
    ...
    @staticmethod
    def delete_public_conversation_by_slug(user: KITUser, slug: str):
...
class ConversationAdapters:
    ...
    def get_conversation_by_user(
        user: KITUser, client_application: ClientApplication = None, conversation_id: str = None
    ...
    @staticmethod
    @require_valid_user
    def get_all_conversations_for_export(user: KITUser, page: Optional[int] = 0):
        all_conversations = Conversation.objects.filter(user=user).prefetch_related("agent")[page : page + 10]
        histories = []
        for conversation in all_conversations:
            history = {
                "title": conversation.title,
                "agent": conversation.agent.name if conversation.agent else "KIT",
    ...
    @staticmethod
    @require_valid_user
    def get_num_conversations(user: KITUser):
        return Conversation.objects.filter(user=user).count()

    @staticmethod
    @require_valid_user
    def get_conversation_sessions(user: KITUser, client_application: ClientApplication = None):
        return (
    ...
    async def aset_conversation_title(
        user: KITUser, client_application: ClientApplication, conversation_id: str, title: str
    ):
    ...
    async def acreate_conversation_session(
        user: KITUser, client_application: ClientApplication = None, agent_slug: str = None, title: str = None
    ):
        if agent_slug:
    ...
    @staticmethod
    @require_valid_user
    def has_any_chat_model(user: KITUser):
    ...
    @staticmethod
    @arequire_valid_user
    async def aset_user_conversation_processor(user: KITUser, conversation_processor_config_id: int):
    ...
    @staticmethod
    @arequire_valid_user
    async def aset_user_voice_model(user: KITUser, model_id: str):
    ...
    @staticmethod
    def get_chat_model(user: KITUser):
    ...
    @staticmethod
    async def aget_chat_model(user: KITUser):
    ...
    @staticmethod
    async def aget_voice_model_config(user: KITUser) -> Optional[VoiceModelOption]:
    ...
    @staticmethod
    def get_voice_model_config(user: KITUser) -> Optional[VoiceModelOption]:
    ...
    @staticmethod
    def get_default_chat_model(user: KITUser = None):
    ...
    async def aget_default_chat_model(
        user: KITUser = None, fallback_chat_model: Optional[ChatModel] = None, fast: Optional[bool] = None
    ):
        """
    ...
    @staticmethod
    def get_advanced_chat_model(user: KITUser):
        server_chat_settings = ServerChatSettings.objects.first()
    ...
    @staticmethod
    async def aget_advanced_chat_model(user: KITUser = None):
        server_chat_settings: ServerChatSettings = (
            await ServerChatSettings.objects.filter()
    ...
    @staticmethod
    def get_max_context_size(chat_model: ChatModel, user: KITUser) -> int | None:
    ...
    @staticmethod
    async def aget_max_context_size(chat_model: ChatModel, user: KITUser) -> int | None:
        """Get the max context size for the user based on the chat model."""
    ...
    @staticmethod
    async def aget_chat_model_slot(user: KITUser = None, fast: Optional[bool] = None):
        """
    ...
    def create_conversation_from_public_conversation(
        user: KITUser, public_conversation: PublicConversation, client_app: ClientApplication
    ):
    ...
    @staticmethod
    def set_user_chat_model(user: KITUser, chat_model: ChatModel):
    ...
    @staticmethod
    async def aget_user_chat_model(user: KITUser):
        config = (
            await UserConversationConfig.objects.filter(user=user).prefetch_related("setting__ai_model_api").afirst()
        )
        if not config:
    ...
    @staticmethod
    async def ais_memory_enabled(user: KITUser) -> bool:
        """
    ...
    @staticmethod
    def is_memory_enabled(user: KITUser) -> bool:
        """
    ...
    @staticmethod
    @arequire_valid_user
    async def aget_conversation_starters(user: KITUser, max_results=3):
        all_questions = []
    ...
    @staticmethod
    async def aget_valid_chat_model(user: KITUser, conversation: Conversation, is_subscribed: bool):
        """
    ...
    @staticmethod
    def get_user_text_to_image_model_config(user: KITUser):
    ...
    @staticmethod
    async def aget_user_text_to_image_model(user: KITUser) -> Optional[TextToImageModelConfig]:
    ...
    @staticmethod
    async def aset_user_text_to_image_model(user: KITUser, text_to_image_model_config_id: int):
    ...
    @staticmethod
    def add_files_to_filter(user: KITUser, conversation_id: str, files: List[str]):
    ...
    @staticmethod
    def remove_files_from_filter(user: KITUser, conversation_id: str, files: List[str]):
        conversation = ConversationAdapters.get_conversation_by_user(user, conversation_id=conversation_id)
        if not conversation:
            return []
        for filename in files:
    ...
    @staticmethod
    @require_valid_user
    def delete_message_by_turn_id(user: KITUser, conversation_id: str, turn_id: str):
        conversation = ConversationAdapters.get_conversation_by_user(user, conversation_id=conversation_id)
...
class FileObjectAdapters:
    ...
    @staticmethod
    @require_valid_user
    def create_file_object(user: KITUser, file_name: str, raw_text: str):
    ...
    @staticmethod
    @require_valid_user
    def get_file_object_by_name(user: KITUser, file_name: str):
        return FileObject.objects.filter(user=user, file_name=file_name).first()

    @staticmethod
    @require_valid_user
    def get_all_file_objects(user: KITUser):
        return FileObject.objects.filter(user=user).all()

    @staticmethod
    @require_valid_user
    def delete_file_object_by_name(user: KITUser, file_name: str):
        return FileObject.objects.filter(user=user, file_name=file_name).delete()

    @staticmethod
    @require_valid_user
    def delete_all_file_objects(user: KITUser):
    ...
    @staticmethod
    @arequire_valid_user
    async def acreate_file_object(user: KITUser, file_name: str, raw_text: str):
        cleaned_raw_text = clean_text_for_db(raw_text)
        return await FileObject.objects.acreate(user=user, file_name=file_name, raw_text=cleaned_raw_text)

    @staticmethod
    @arequire_valid_user
    async def aget_file_objects_by_name(user: KITUser, file_name: str, agent: Agent = None):
        return await sync_to_async(list)(FileObject.objects.filter(user=user, file_name=file_name, agent=agent))

    @staticmethod
    @arequire_valid_user
    async def aget_file_objects_by_path_prefix(user: KITUser, path_prefix: str, agent: Agent = None):
...
class EntryAdapters:
    ...
    @staticmethod
    @require_valid_user
    def does_entry_exist(user: KITUser, hashed_value: str) -> bool:
        return Entry.objects.filter(user=user, hashed_value=hashed_value).exists()

    @staticmethod
    @require_valid_user
    def delete_entry_by_file(user: KITUser, file_path: str):
        deleted_count, _ = Entry.objects.filter(user=user, file_path=file_path).delete()
        return deleted_count

    @staticmethod
    @require_valid_user
    def get_filtered_entries(user: KITUser, file_type: str = None, file_source: str = None):
    ...
    @staticmethod
    @require_valid_user
    def delete_all_entries(user: KITUser, file_type: str = None, file_source: str = None, batch_size=1000):
        deleted_count = 0
    ...
    @staticmethod
    @arequire_valid_user
    async def adelete_all_entries(user: KITUser, file_type: str = None, file_source: str = None, batch_size=1000):
        deleted_count = 0
        queryset = EntryAdapters.get_filtered_entries(user, file_type, file_source)
    ...
    @staticmethod
    @require_valid_user
    def get_existing_entry_hashes_by_file(user: KITUser, file_path: str):
        return Entry.objects.filter(user=user, file_path=file_path).values_list("hashed_value", flat=True)

    @staticmethod
    @require_valid_user
    def delete_entry_by_hash(user: KITUser, hashed_values: List[str]):
    ...
    @staticmethod
    @require_valid_user
    def user_has_entries(user: KITUser):
    ...
    @staticmethod
    @arequire_valid_user
    async def auser_has_entries(user: KITUser):
    ...
    @staticmethod
    @arequire_valid_user
    async def adelete_entry_by_file(user: KITUser, file_path: str):
        return await Entry.objects.filter(user=user, file_path=file_path).adelete()

    @staticmethod
    @arequire_valid_user
    async def adelete_entries_by_filenames(user: KITUser, filenames: List[str], batch_size=1000):
        deleted_count = 0
    ...
    @staticmethod
    @require_valid_user
    def get_all_filenames_by_source(user: KITUser, file_source: str):
        return (
    ...
    @staticmethod
    @require_valid_user
    def get_all_filenames_by_type(user: KITUser, file_type: str):
        return (
    ...
    @staticmethod
    @require_valid_user
    def get_size_of_indexed_data_in_mb(user: KITUser):
        entries = Entry.objects.filter(user=user).iterator()
        total_size = sum(sys.getsizeof(entry.compiled) for entry in entries)
        return total_size / 1024 / 1024

    @staticmethod
    def apply_filters(user: KITUser, query: str, file_type_filter: str = None, agent: Agent = None):
        q_filter_terms = Q()
...
class AutomationAdapters:
    ...
    @staticmethod
    def get_automation_metadata(user: KITUser, automation: Job):
    ...
    @staticmethod
    def get_job_last_run(user: KITUser, automation: Job):
        # Perform validation checks
        # Check if user is allowed to delete this automation id
        if not automation.id.startswith(f"automation_{user.uuid}_"):
            raise ValueError(f"Invalid automation id: {automation.id}")
    ...
    @staticmethod
    def get_automations_metadata(user: KITUser):
    ...
    @staticmethod
    def get_automation(user: KITUser, automation_id: str) -> Job:
        # Perform validation checks
    ...
    @staticmethod
    async def aget_automation(user: KITUser, automation_id: str) -> Job:
        # Perform validation checks
        # Check if user is allowed to retrieve this automation id
        if is_none_or_empty(automation_id) or not automation_id.startswith(f"automation_{user.uuid}_"):
            raise ValueError(f"Invalid automation id: {automation_id}")
    ...
    @staticmethod
    def delete_automation(user: KITUser, automation_id: str):
...
class UserMemoryAdapters:
    @staticmethod
    @require_valid_user
    async def pull_memories(user: KITUser, agent: Agent = None, limit=10, window=7) -> list[UserMemory]:
        """
    ...
    @staticmethod
    @require_valid_user
    async def save_memory(user: KITUser, memory: str, agent: Agent = None) -> UserMemory:
        """
    ...
    @staticmethod
    @require_valid_user
    async def search_memories(query: str, user: KITUser, agent: Agent = None, limit: int = 10) -> list[UserMemory]:
        """
    ...
    @staticmethod
    @require_valid_user
    async def delete_memory(user: KITUser, memory_id: str) -> bool:
        """
