...
def cli(args=None):
    ...
    parser.add_argument("--version", "-V", action="store_true", help="Print the installed KIT version and exit")
    parser.add_argument(
        "--anonymous-mode",
        action="store_true",
        default=False,
        help="Run KIT in single user mode with no login required. Useful for personal use or testing.",
    )
    parser.add_argument(
        "--non-interactive",
        action="store_true",
        default=False,
        help="Start KIT in non-interactive mode. Assumes interactive shell unavailable for config. E.g when run via Docker.",
    )
    ...
    args.version_no = version("KIT")
    if args.version:
        # Show version of KIT installed and exit
