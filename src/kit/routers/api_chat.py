...
from kit.app.settings import ALLOWED_HOSTS
...
from kit.database.models import Agent, KITUser
from kit.processor.conversation import prompts
from kit.processor.conversation.openai.utils import is_local_api
from kit.processor.conversation.prompts import no_entries_found
...
from kit.processor.image.generate import text_to_image
from kit.processor.operator import operate_environment
from kit.processor.speech.text_to_speech import generate_text_to_speech
from kit.processor.tools.online_search import (
    deduplicate_organic_results,
    read_webpages,
    search_online,
)
from kit.processor.tools.run_code import run_code
from kit.routers.email import send_query_feedback
...
from kit.routers.research import ResearchIteration, research
from kit.routers.storage import upload_user_image_to_bucket
...
def duplicate_chat_history_public_conversation(
    ...
    if scheme == "http" and not is_env_var_true("KIT_NO_HTTPS") and not is_local_api(f"{request.base_url}"):
        scheme = "https"
...
async def event_generator(
    ...
    chat_metadata: dict = {}
    conversation = None
    user: KITUser = user_scope.object
