...
from kit.app.settings import DISABLE_HTTPS
...
from kit.routers.email import send_magic_link_email, send_welcome_email
...
from kit.utils.helpers import in_debug_mode
...
if not state.anonymous_mode:
    ...
    if missing_requirements:
        ...
        error_msg = f"🚨 Start KIT with --anonymous-mode flag or to enable authentication:{requirements_string}"
...
async def generate_token(request: Request, token_name: Optional[str] = None):
    ...
    if token_name:
        token = await acreate_KIT_token(user=request.user.object, name=token_name)
    else:
        token = await acreate_KIT_token(user=request.user.object)
    return {
        "token": token.token,
        "name": token.name,
    }
...
def get_tokens(request: Request):
    ...
    tokens = get_KIT_tokens(user=request.user.object)
    return tokens
...
async def delete_token(request: Request, token: str):
    ...
    return await delete_KIT_token(user=request.user.object, token=token)
...
async def auth_post(request: Request):
    ...
    KIT_user = await get_or_create_user(idinfo)

    if KIT_user:
        request.session["user"] = dict(idinfo)

        if datetime.timedelta(minutes=3) > (datetime.datetime.now(datetime.timezone.utc) - KIT_user.date_joined):
            asyncio.create_task(send_welcome_email(idinfo["name"], idinfo["email"]))
            update_telemetry_state(
                request=request,
                telemetry_type="api",
                api="create_user__google",
                metadata={"server_id": str(KIT_user.uuid)},
            )
            logger.log(logging.INFO, f"🥳 New User Created: {KIT_user.uuid}")
