...
from kit.utils.helpers import in_debug_mode, is_env_var_true
...
# Initialize Django
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "kit.app.settings")
django.setup()
...
# Initialize Django Database and Static Files (skippable)
db_migrate_output = io.StringIO()
collectstatic_output = io.StringIO()
if not is_env_var_true("KIT_SKIP_AUTO_MIGRATE"):
    ...
    db_migrate_output.write("Auto-migrate skipped via KIT_SKIP_AUTO_MIGRATE")
    collectstatic_output.write("Auto-collectstatic skipped via KIT_SKIP_AUTO_MIGRATE")
...
from kit.configure import configure_routes, initialize_server, configure_middleware
from kit.utils import state
from kit.utils.cli import cli
from kit.utils.initialization import initialization
...
def run(should_start_server=True):
    ...
    logger.info(f"🚒 Initializing KIT v{state.KIT_version}")
...
def set_state(args):
    ...
    state.KIT_version = version("KIT")


def start_server(app, host=None, port=None, socket=None):
    logger.info("🌖 KIT is ready to engage")
    if socket:
        uvicorn.run(
            app,
            proxy_headers=True,
