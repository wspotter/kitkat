# KIT Operator (Experimental)

## Overview
Give KIT its own computer to operate in a transparent, controlled manner. Accomplish tasks that require visual browsing, file editing and terminal access. Operator with research mode can work for 30+ minutes to accomplish more substantial tasks like feature development, travel planning, shopping etc.
...
### Installation Steps
1. Download the KIT docker-compose.yml file
    ```shell
    mkdir ~/.KIT && cd ~/.KIT
    wget https://raw.githubusercontent.com/KIT-ai/KIT/master/docker-compose.yml
    ```
...
2. Update the `docker-compose.yml` to enable computer operator
    ...
    - Uncomment `KIT_OPERATOR_ENABLED=True` to enable the operator tool
    - Uncomment `- /var/run/docker.sock:/var/run/docker.sock` to mount docker socket to allow KIT to operate its computer container.

3. Start KIT services
    ```shell
    docker-compose up
    ```
...
## Usage
Use the `/operator` command or ask KIT in normal or research mode to use the operator tool to have it operate its computer:

**Examples:**
- `/operator Find flights from Bangkok to Mexico City with no US layover`
- `/research Clone the KIT repo and tell me how the operator tool is implemented`
