...
class BinaryOperatorAgent(OperatorAgent):
    ...
    def _format_message_for_api(self, messages: list[AgentMessage]) -> List[ChatMessageModel]:
        """Format operator agent messages into the KIT conversation history format."""
        formatted_messages = [
            ChatMessageModel(
    ...
    def get_instruction(self, environment_type: EnvironmentType, env_state: EnvState) -> str:
        ...
        if environment_type == EnvironmentType.BROWSER:
            return dedent(
                f"""
                # Introduction
                * You are KIT, a smart and resourceful web browsing assistant. You help the user accomplish their task using a web browser.
            ...
            return dedent(
                f"""
                # Introduction
                * You are KIT, a smart and resourceful computer assistant. You help the user accomplish their task using a computer.
