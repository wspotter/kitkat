...
const KIT_URL = 'https://app.KIT.dev';

const Store = require('electron-store');

const textFileTypes = [
    // Default valid file extensions supported by KIT
...
function pushDataToKIT (regenerate = false) {
    // Don't sync if token or hostURL is not set or if already syncing
    if (store.get('KITToken') === '' || store.get('hostURL') === '' || syncing === true) {
    ...
    for (const file of filesToPush) {
        ...
        // Collect all updated or newly created files since last sync to index on KIT server
        try {
    ...
    // Mark deleted files for removal from index on KIT server
    ...
    // Send collected files to KIT server for indexing
    const hostURL = store.get('hostURL') || KIT_URL;
    const headers = { 'Authorization': `Bearer ${store.get("KITToken")}` };
    let requests = [];
    ...
    .catch(error => {
        ...
        if (error?.response?.status === 429 && (BrowserWindow.getAllWindows().find(win => win.webContents.getURL().includes('settings')))) {
            state["error"] = `Looks like you're out of space to sync your files. <a href="https://app.KIT.dev/settings#subscription">Upgrade your plan</a> to unlock more space.`;
            ...
            state["error"] = `Could not connect to KIT server. Ensure you can connect to it at ${error.address}:${error.port}.`;
        } else {
            currentTime = new Date();
            state["error"] = `Sync was unsuccessful at ${currentTime.toLocaleTimeString()}. Contact team@KIT.dev to report this issue.`;
        }
    })
    .finally(() => {
        // Syncing complete
...
async function handleFileOpen (type) {
    ...
    if (type === 'file') {
        ({ canceled, filePaths } = await dialog.showOpenDialog({properties: ['openFile' ], filters: [{ name: "Valid KIT Files", extensions: validFileTypes }] }));
    } else if (type === 'folder') {
...
async function getToken () {
    return store.get('KITToken');
}

async function setToken (event, token) {
    store.set('KITToken', token);
    return store.get('KITToken');
}
...
async function syncData (regenerate = false) {
    try {
        pushDataToKIT(regenerate);
        const date = new Date();
        console.log('Pushing data to KIT at: ', date);
    } catch (err) {
        console.error(err);
    }
}
...
// Fetch user info from KIT server
async function getUserInfo() {
    const getUserInfoURL = `${store.get('hostURL') || KIT_URL}/api/v1/user?client=desktop`;
    const headers = { 'Authorization': `Bearer ${store.get('KITToken')}` };
    try {
...
function addCSPHeaderToSession () {
    ...
    const defaultDomains = `'self' ${hostURL} https://app.KIT.dev https://assets.KIT.dev`;
    ...
    const img_src = `img-src ${defaultDomains} data: https://*.KIT.dev https://*.googleusercontent.com;`;
...
app.whenReady().then(() => {
    ...
    app.setAboutPanelOptions({
        applicationName: "KIT",
        applicationVersion: KITPackage.version,
        version: KITPackage.version,
        authors: "KIT AI",
        website: "https://KIT.dev",
        copyright: "GPL v3",
...
function openAboutWindow() {
    ...
    // Pass OS, KIT version to About page
    aboutWindow.webContents.on('did-finish-load', () => {
        aboutWindow.webContents.send('appInfo', { version: KITPackage.version, platform: process.platform });
    });
...
app.whenReady().then(() => {
    ...
    const contextMenu = Menu.buildFromTemplate([
        ...
        { type: 'separator' },
        { label: 'About KIT', type: 'normal', click: () => { openAboutWindow(); } },
