...
window.needsSubscriptionAPI.onNeedsSubscription((event, needsSubscription) => {
    ...
    if (needsSubscription) {
        window.alert("Looks like you're out of space to sync your files. Upgrade your plan to unlock more space here: https://app.KIT.dev/settings#subscription");
        needsSubscriptionElement.style.display = 'block';
    }
});

const urlInput = document.getElementById('KIT-host-url');
(async function () {
...
const KITKeyInput = document.getElementById('KIT-access-key');
(async function () {
    const token = await window.tokenAPI.getToken();
    KITKeyInput.value = token;
})();

KITKeyInput.addEventListener('blur', async () => {
    const token = await window.tokenAPI.setToken(KITKeyInput.value.trim());
    KITKeyInput.value = token;
});
