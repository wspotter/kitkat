;;; KIT-tests.el --- Test suite for KIT.el  -*- lexical-binding: t -*-
...
;; This file contains the test suite for KIT.el.
...
")))
    (should
     (equal
      (KIT--extract-entries-as-markdown json-response-from-KIT-backend user-query nil)
      "\
# Become God\n\
## Upgrade\n\
\n\
...
(ert-deftest KIT-tests--extract-entries-as-org ()
...
")))
    (should
     (equal
      (KIT--extract-entries-as-org json-response-from-KIT-backend user-query nil)
      "\
* Become God\n\
** Upgrade\n\
\n\
...
(ert-deftest KIT-tests--get-current-outline-entry-text ()
...
(ert-deftest KIT-tests--get-current-paragraph-text ()
...
(ert-deftest KIT-tests--render-files-to-add-request-body ()
  ...
  (let ((upgrade-file (make-temp-file "upgrade" nil ".org" "# Become God\n## Upgrade\n\nPenance to Immortality\n\n"))
    ...
    (unwind-protect
        (progn
          (should
           (equal
            (KIT--render-update-files-as-request-body (list upgrade-file act-file) "KIT")
            (format
             "\n--KIT\r\n\
...
(ert-deftest KIT-tests--render-files-to-add-delete-in-request-body ()
  "Test files are formatted into a multi-part http request body"
  (let ((upgrade-file (make-temp-file "upgrade" nil ".org" "# Become God\n## Upgrade\n\nPenance to Immortality\n\n"))
        (act-file (make-temp-file "act" nil ".org" "## Act\n\nRule everything\n\n")))
    (unwind-protect
        (progn
          (should
           (equal
            (KIT--render-delete-files-as-request-body (list upgrade-file act-file "/tmp/deleted-file.org") "KIT")
            (format
             "\n--KIT\r\n\
Content-Disposition: form-data; name=\"files\"; filename=\"%s\"\r\n\
Content-Type: text/org\r\n\r\n\
...
(ert-deftest KIT-tests--extract-online-references ()
  (let* (;; Arrange
         ...
         ;; Act
         (result (KIT--extract-online-references
