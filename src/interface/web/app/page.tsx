...
function ChatBodyData(props: ChatBodyDataProps) {
    ...
    const [selectedAgent, setSelectedAgent] = useState<string | null>("KIT");
    ...
    useEffect(() => {
        ...
        setSelectedAgent(agents.length > 1 ? agents[0].slug : "KIT");
    ...
    useEffect(() => {
        const processMessage = async () => {
            if (message && !processingMessage) {
                ...
                try {
                    const newConversationId = await createNewConversation(selectedAgent || "KIT");
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger>
                                <Button className="w-full m-2 p-0" variant="outline">
                                    ...
                                    {selectedAgent
                                        ? `${agents?.find((agent) => agent.slug === selectedAgent)?.name ?? "KIT"}`
                                        : "Select Agent"}
                                </Button>
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="overflow-y-scroll max-h-96">
                                {agentIcons.length > 0 ? (
                                    ...
                                    <DropdownMenuItem
                                        key="0-KIT"
                                        onClick={() => {
                                            setSelectedAgent("KIT");
                                            chatInputRef.current?.focus();
                                        }}
                                    >
                                        {getIconFromIconName("Lightbulb", "orange")} KIT
                                    </DropdownMenuItem>
                                )}
                            </DropdownMenuContent>
                        </DropdownMenu>
        ...
        <SidebarProvider>
            ...
            <SidebarInset>
                ...
                <div className={`${styles.main} ${styles.chatLayout}`}>
                    <title>KIT AI - Your Second Brain</title>
