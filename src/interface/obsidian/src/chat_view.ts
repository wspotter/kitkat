...
import { KITView, createCopyParentText, getLinkToEntry, pasteTextAtCursor } from 'src/utils';
...
export class KITChatView extends KITPaneView {
    result: string;
    ...
    constructor(leaf: WorkspaceLeaf, plugin: KIT) {
        ...
        this.scope.register(["Ctrl"], 'f', (_) => new KITSearchModal(this.app, this.setting).open());
        this.scope.register(["Ctrl"], 'r', (_) => { this.activateView(KITView.SIMILAR); });
    }
    ...
    async chat(isVoice: boolean = false) {
        ...
        let input_el = <HTMLTextAreaElement>this.contentEl.getElementsByClassName("KIT-chat-input")[0];
        ...
        if (user_message) {
            ...
            const selectedMode = this.chatModes.find(mode =>
                this.contentEl.querySelector(`#KIT-mode-${mode.value}:checked`)
            );
    ...
    async onOpen() {
        ...
        let defaultDomains = `'self' ${this.setting.KITUrl} https://*.obsidian.md https://app.KIT.dev https://assets.KIT.dev`;
        const defaultSrc = `default-src ${defaultDomains};`;
        const scriptSrc = `script-src ${defaultDomains} 'unsafe-inline';`;
        const connectSrc = `connect-src ${this.setting.KITUrl} wss://*.obsidian.md/ https://ipapi.co/json;`;
        ...
        // WARNING: CSP DISABLED for now as it breaks other Obsidian plugins. Enable when can scope CSP to only KIT plugin.
        // CSP meta tag for the KIT Chat modal
        ...
        const headerAgentSelect = this.contentEl.querySelector('.KIT-header-agent-select') as HTMLSelectElement;
        if (headerAgentSelect && this.agents.length > 0) {
            ...
            this.agents.forEach(agent => {
                if (agent.slug === 'KIT') return; // Skip the default agent
        ...
        contentEl.addClass("KIT-chat");

        // Create the chat body
        let chatBodyEl = contentEl.createDiv({ attr: { id: "KIT-chat-body", class: "KIT-chat-body" } });
        // Add chat input field
        let inputRow = contentEl.createDiv("KIT-input-row");

        let chatSessions = inputRow.createEl("button", {
            text: "Chat Sessions",
            attr: {
                class: "KIT-input-row-button clickable-icon",
        ...
        let transcribe = inputRow.createEl("button", {
            ...
            attr: {
                id: "KIT-transcribe",
                class: "KIT-transcribe KIT-input-row-button clickable-icon ",
        ...
        let send = inputRow.createEl("button", {
            ...
            attr: {
                id: "KIT-chat-send",
                class: "KIT-chat-send KIT-input-row-button clickable-icon",
            },
        })
        ...
        // Get chat history from KIT backend and set chat input state
        let getChatHistorySucessfully = await this.getChatHistory(chatBodyEl);

        let placeholderText: string = getChatHistorySucessfully ? this.startingMessage : "Configure KIT to enable chat";
        ...
        requestAnimationFrame(() => {
            ...
            requestAnimationFrame(() => {
                ...
                const chatInput = <HTMLTextAreaElement>this.contentEl.getElementsByClassName("KIT-chat-input")[0];
                chatInput?.focus();
            });
        });
    }
        ...
        if (!this.keyPressTimeout) {
            this.keyPressTimeout = setTimeout(async () => {
                ...
                if (this.sendMessageTimeout) {
                    ...
                    const sendButton = <HTMLButtonElement>this.contentEl.getElementsByClassName("KIT-chat-send")[0]
    ...
    textToSpeech(message: string, event: MouseEvent | null = null): void {
        ...
        let textToSpeechApi = `${this.setting.KITUrl}/api/chat/speech?text=${encodeURIComponent(message)}`;
        fetch(textToSpeechApi, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${this.setting.KITApiKey}`,
            },
        })
    ...
    markdownTextToSanitizedHtml(markdownText: string, component: ItemView): string {
        ...
        virtualChatMessageBodyTextEl.innerHTML = virtualChatMessageBodyTextEl.innerHTML.replace(
            /<img(?:(?!src=["'](app:|data:|https:\/\/generated\.KIT\.dev)).)*?>/gis,
            ''
        );
    ...
    renderMessageWithReferences(
        ...
        let chatMessageBodyEl = chatMessageEl.getElementsByClassName("KIT-chat-message-text")[0];
    ...
    generateImageMarkdown(message: string, intentType: string, inferredQueries?: string[], conversationId?: string, images?: string[], excalidrawDiagram?: string, mermaidjsDiagram?: string): string {
        ...
        if (intentType === "text-to-image") {
            ...
            const domain = this.setting.KITUrl.endsWith("/") ? this.setting.KITUrl : `${this.setting.KITUrl}/`;
    ...
    renderMessage({ chatBodyEl, message, sender, dt, turnId, raw = false, willReplace = true, isSystemMessage = false }: RenderMessageOptions): Element {
        ...
        let chatMessageEl = chatBodyEl.createDiv({
            attr: {
                ...
                class: `KIT-chat-message ${sender}`,
                ...(turnId && { "data-turnid": turnId })
            },
        })
        let chatMessageBodyEl = chatMessageEl.createDiv();
        chatMessageBodyEl.addClasses(["KIT-chat-message-text", sender]);
    ...
    createKITResponseDiv(dt?: Date): HTMLDivElement {
        let messageTime = this.formatDate(dt ?? new Date());

        // Append message to conversation history HTML element.
        // The chat logs should display above the message input box to follow standard UI semantics
        let chatBodyEl = this.contentEl.getElementsByClassName("KIT-chat-body")[0];
        let chatMessageEl = chatBodyEl.createDiv({
            attr: {
                "data-meta": messageTime,
                class: `KIT-chat-message KIT`
            },
        })
    ...
    renderActionButtons(message: string, chatMessageBodyTextEl: HTMLElement, isSystemMessage: boolean = false) {
        ...
        let editButton = null;
        if (!isSystemMessage && chatMessageBodyTextEl.closest('.KIT-chat-message.you')) {
            ...
            editButton.addEventListener('click', async () => {
                const messageEl = chatMessageBodyTextEl.closest('.KIT-chat-message');
                if (messageEl) {
                    // Get all messages up to this one
                    const allMessages = Array.from(this.contentEl.getElementsByClassName('KIT-chat-message'));
                    ...
                    const chatInput = this.contentEl.querySelector('.KIT-chat-input') as HTMLTextAreaElement;
                    if (chatInput) {
    ...
    async createNewConversation(agentSlug?: string | null) {
        let chatBodyEl = this.contentEl.getElementsByClassName("KIT-chat-body")[0] as HTMLElement;
        ...
        try {
            ...
            let endpoint = `${this.setting.KITUrl}/api/chat/sessions`;
            ...
            if (response.ok) {
                ...
                const agentSelect = this.contentEl.querySelector('.KIT-header-agent-select') as HTMLSelectElement;
                if (agentSelect) {
        ...
        this.renderMessage({ chatBodyEl, message: "Hey, what's up?", sender: "KIT", isSystemMessage: true });
    }
    ...
    async toggleChatSessions(forceShow: boolean = false): Promise<boolean> {
        ...
        const chatSessionsUrl = `${this.setting.KITUrl}/api/chat/sessions?client=obsidian`;
        const headers = { 'Authorization': `Bearer ${this.setting.KITApiKey}` };
        try {
    ...
    addConversationMenu(
        ...
        editConversationTitleButtonEl.addEventListener('click', (event) => {
            ...
            editConversationTitleSaveButtonEl.addEventListener('click', (event) => {
                ...
                if (newTitle != null) {
                    ...
                    fetch(`${this.setting.KITUrl}${editURL}`, { method: "PATCH", headers })
        ...
        deleteConversationButtonEl.addEventListener('click', () => {
            ...
            fetch(`${this.setting.KITUrl}${deleteURL}`, { method: "DELETE", headers })
                .then(response => response.ok ? response.json() : Promise.reject(response))
                .then(data => {
                    chatBodyEl.innerHTML = "";
                    chatBodyEl.dataset.conversationId = "";
    ...
    async getChatHistory(chatBodyEl: HTMLElement): Promise<boolean> {
        // Get chat history from KIT backend
        let chatUrl = `${this.setting.KITUrl}/api/chat/history?client=obsidian`;
        ...
        try {
            let response = await fetch(chatUrl, {
                method: "GET",
                headers: { "Authorization": `Bearer ${this.setting.KITApiKey}` },
            });
            ...
            if (responseJson.detail) {
                ...
                chatLogs.forEach((chatLog: any) => {
                    ...
                    // Transform KIT-edit blocks into accordions
                    if (chatLog.by === "KIT") {
            ...
            let errorMsg = "Unable to get response from KIT server ❤️‍🩹. Ensure server is running or contact developers for help at [team@KIT.dev](mailto:team@KIT.dev) or in [Discord](https://discord.gg/BDgyabRM6e)";
            this.renderMessage({
                chatBodyEl,
                message: errorMsg,
                sender: "KIT",
                ...
                if (editBlocks.length > 0) {
                    ...
                    if (firstBlock.hasError) {
                        ...
                        if (this.editRetryCount < this.maxEditRetries) {
                            ...
                            console.warn('[KIT] Max edit retries reached. Aborting further retries.');
                        }
                    } else {
    ...
    async getChatResponse(query: string | undefined | null, displayQuery: string | undefined | null, isVoice: boolean = false, displayUserMessage: boolean = true): Promise<void> {
        ...
        if (!conversationId) {
            try {
                ...
                const response = await fetch(`${this.setting.KITUrl}/api/chat/sessions`, {
                    method: "POST",
                    headers: {
                        "Authorization": `Bearer ${this.setting.KITApiKey}`,
                        "Content-Type": "application/json"
        ...
        // Get chat response from KIT backend
        const chatUrl = `${this.setting.KITUrl}/api/chat?client=obsidian`;
        const body = {
            q: finalQuery,
        ...
        let newResponseEl = this.createKITResponseDiv();
        let newResponseTextEl = newResponseEl.createDiv();
        newResponseTextEl.classList.add("KIT-chat-message-text", "KIT");

        // Temporary status message to indicate that KIT is thinking
        ...
        try {
            ...
            console.error(`KIT chat response failed with\n${err}`);
            let errorMsg = "Sorry, unable to get response from KIT backend ❤️‍🩹. Retry or contact developers for help at <a href=mailto:'team@KIT.dev'>team@KIT.dev</a> or <a href='https://discord.gg/BDgyabRM6e'>on Discord</a>";
    ...
    async clearConversationHistory() {
        let chatBody = this.contentEl.getElementsByClassName("KIT-chat-body")[0] as HTMLElement;

        let response = await request({
            url: `${this.setting.KITUrl}/api/chat/history?client=obsidian`,
            method: "DELETE",
            headers: { "Authorization": `Bearer ${this.setting.KITApiKey}` },
        })
        try {
    ...
    async speechToText(event: MouseEvent | TouchEvent | KeyboardEvent) {
        event.preventDefault();
        const transcribeButton = <HTMLButtonElement>this.contentEl.getElementsByClassName("KIT-transcribe")[0];
        const chatInput = <HTMLTextAreaElement>this.contentEl.getElementsByClassName("KIT-chat-input")[0];
        const sendButton = <HTMLButtonElement>this.contentEl.getElementsByClassName("KIT-chat-send")[0]
        ...
        const sendToServer = async (audioBlob: Blob) => {
            ...
            const response = await requestUrl({
                url: `${this.setting.KITUrl}/api/transcribe?client=obsidian`,
                method: 'POST',
                headers: { "Authorization": `Bearer ${this.setting.KITApiKey}` },
            ...
            // Parse response from KIT backend
    ...
    cancelSendMessage() {
        ...
        let sendButton = <HTMLButtonElement>this.contentEl.getElementsByClassName("KIT-chat-send")[0];
        setIcon(sendButton, "arrow-up-circle");
        let sendImg = <SVGElement>sendButton.getElementsByClassName("lucide-arrow-up-circle")[0]
        sendImg.addEventListener('click', async (_) => { await this.chat() });
    };
    ...
    incrementalChat(event: KeyboardEvent) {
        ...
        if (this.modeDropdown && this.modeDropdown.style.display !== "none" && event.key === "Enter") {
            ...
            const options = this.modeDropdown.querySelectorAll<HTMLElement>(".KIT-mode-dropdown-option");
    ...
    scrollChatToBottom() {
        const chat_body_el = this.contentEl.getElementsByClassName("KIT-chat-body")[0];
    ...
    handleStreamResponse(newResponseElement: HTMLElement | null, rawResponse: string, loadingEllipsis: HTMLElement | null, replace = true) {
        ...
        messageEl.classList.add('KIT-message-new-content');
        newResponseElement.appendChild(messageEl);

        // Remove the animation class after the animation completes
        setTimeout(() => {
            newResponseElement.classList.remove('KIT-message-new-content');
        }, 300);
    }
    ...
    finalizeChatBodyResponse(references: object, newResponseElement: HTMLElement | null, turnId: string) {
        ...
        this.scrollChatToBottom();
        let chatInput = this.contentEl.getElementsByClassName("KIT-chat-input")[0];
    ...
    private highlightVisibleOption(visibleOptions: HTMLElement[]) {
        const allOptions = this.modeDropdown?.querySelectorAll<HTMLElement>(".KIT-mode-dropdown-option");
        if (!allOptions) return;

        // Clear highlighting on all options first
        allOptions.forEach(option => {
            option.classList.remove("KIT-mode-dropdown-option-selected");
        });

        // Add highlighting to the selected visible option
        if (this.selectedOptionIndex >= 0 && this.selectedOptionIndex < visibleOptions.length) {
            const selectedOption = visibleOptions[this.selectedOptionIndex];
            selectedOption.classList.add("KIT-mode-dropdown-option-selected");
    ...
    async deleteMessage(messageEl: HTMLElement, skipPaired: boolean = false, skipBackend: boolean = false) {
        ...
        const messageContainer = messageEl.closest('.KIT-chat-message');
        if (!messageContainer) return;

        // Get paired message to delete if needed
        let pairedMessageContainer: Element | null = null;
        if (!skipPaired) {
            const messages = Array.from(document.getElementsByClassName('KIT-chat-message'));
            const currentIndex = messages.indexOf(messageContainer as HTMLElement);

            // If we're deleting a user message, also delete the subsequent KIT message (if any)
            if (messageContainer.classList.contains('you') && currentIndex < messages.length - 1) {
                pairedMessageContainer = messages[currentIndex + 1];
            }
            // If we're deleting a KIT message, also delete the preceding user message (if any)
            else if (messageContainer.classList.contains('KIT') && currentIndex > 0) {
                pairedMessageContainer = messages[currentIndex - 1];
            }
        }
        ...
        setTimeout(async () => {
            ...
            if (!skipBackend && turnId) {
                const chatBodyEl = this.contentEl.getElementsByClassName("KIT-chat-body")[0] as HTMLElement;
                ...
                try {
                    // Delete from backend
                    const response = await fetch(`${this.setting.KITUrl}/api/chat/conversation/message`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${this.setting.KITApiKey}`
    ...
    async fetchAgents() {
        try {
            const response = await fetch(`${this.setting.KITUrl}/api/agents`, {
                headers: {
                    "Authorization": `Bearer ${this.setting.KITApiKey}`
                }
            });
    ...
    private async applyEditBlocks(editBlocks: EditBlock[]) {
        ...
        const chatBodyEl = this.contentEl.getElementsByClassName("KIT-chat-body")[0];
    ...
    // Add this new method to handle KIT-edit block transformation
    ...
    private async handleEditRetry(errorBlock: EditBlock) {
        ...
        const retryContainer = chatBodyEl.createDiv({ cls: "KIT-retry-container" });

        // Create the retry badge inside the container
        const retryBadge = retryContainer.createDiv({ cls: "KIT-retry-badge" });
        ...
        if (!this.modeDropdown) {
            ...
            this.chatModes.forEach((mode, index) => {
                ...
                const emojiSpan = option.createSpan({
                    cls: "KIT-mode-dropdown-emoji"
                });
                setIcon(emojiSpan, mode.iconName);

                option.createSpan({
                    cls: "KIT-mode-dropdown-label",
                    text: ` ${mode.label} `
                });

                option.createSpan({
                    cls: "KIT-mode-dropdown-command",
                    text: `(${mode.command})`
                });
