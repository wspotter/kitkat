...
export class KITSearchModal extends SuggestModal<SearchResult> {
    setting: KITSetting;
    ...
    constructor(app: App, setting: KITSetting, find_similar_notes: boolean = false) {
        super(app);
        this.app = app;
        ...
        this.scope.register(['Mod'], 'Enter', async () => {
            ...
            this.rerank = true
            // Trigger input event to get and render (reranked) results from KIT backend
        ...
        this.setPlaceholder('Search with KIT...');
    }
    ...
    async getSuggestions(query: string): Promise<SearchResult[]> {
        ...
        try {
            ...
            let searchUrl = `${this.setting.KITUrl}/api/search?q=${encodedQuery}&n=${this.setting.resultsCount}&r=${this.rerank}&client=obsidian`;
            let headers = {
    ...
    async onOpen() {
        if (this.find_similar_notes) {
            ...
            if (file && file.extension === 'md') {
                ...
                // Trigger search to get and render similar notes from KIT backend
                this.inputEl.dispatchEvent(new Event('input'));
                this.rerank = false
            }
            else {
    ...
    async renderSuggestion(result: SearchResult & { inVault: boolean }, el: HTMLElement) {
        ...
        let result_el = el.createEl("div", { cls: 'KIT-result-entry' })
