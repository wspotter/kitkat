...
/**
 * Interface representing the result of parsing a KIT edit block
 */
...
/**
 * Class that handles file operations for the KIT plugin
 */
export class FileInteractions {
    private app: App;
    private readonly EDIT_BLOCK_START = '<KIT_edit>';
    private readonly EDIT_BLOCK_END = '</KIT_edit>';
...
`;
    ...
    public createPreviewWithDiff(originalText: string, newText: string): string {
        ...
        const HIGHLIGHT_TOKEN = "___KIT_HIGHLIGHT_MARKER___";
        const STRIKETHROUGH_TOKEN = "___KIT_STRIKETHROUGH_MARKER___";
    ...
    public transformEditBlocks(message: string): string {
        ...
        for (const block of partialBlocks) {
            ...
            if (error) {
                // Error block
                console.error("Error parsing KIT-edit block:", error);
                console.error("Content causing error:", content);
                ...
                htmlRender = `<details class="KIT-edit-accordion error">
                    <summary>${errorTitle}</summary>
                    <div class="KIT-edit-content">
                        <p class="KIT-edit-error-message">${errorDetails}</p>
                        <pre><code class="language-md error">${diffContent}</code></pre>
                    </div>
                </details>`;
            } else if (editData && inProgress) {
                // In-progress block
                htmlRender = `<details class="KIT-edit-accordion in-progress">
                    <summary>📄 ${editData.file} <span class="KIT-edit-status">In Progress</span></summary>
                    <div class="KIT-edit-content">
                ...
                htmlRender = `<details class="KIT-edit-accordion success">
