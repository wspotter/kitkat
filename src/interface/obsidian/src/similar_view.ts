...
import { KITView, getLinkToEntry, supportedBinaryFileTypes } from 'src/utils';
...
export class KITSimilarView extends KITPaneView {
    ...
    getViewType(): string {
        return KITView.SIMILAR;
    }

    getDisplayText(): string {
        return "KIT Similar Documents";
    }
    ...
    async onOpen() {
        ...
        const mainContainerEl = contentEl.createDiv({ cls: "KIT-similar-container" });

        // Create search input container
        const searchContainerEl = mainContainerEl.createDiv({ cls: "KIT-similar-search-container" });

        // Create search input
        this.searchInputEl = searchContainerEl.createEl("input", {
            cls: "KIT-similar-search-input",
            attr: {
                type: "text",
        ...
        const refreshButtonEl = searchContainerEl.createEl("button", {
            cls: "KIT-similar-refresh-button"
        });
        ...
        this.resultsContainerEl = mainContainerEl.createDiv({ cls: "KIT-similar-results" });
    ...
    /**
     * Get similar documents from KIT API
     *
    ...
    async getSimilarDocuments(query: string): Promise<SimilarResult[]> {
        ...
        try {
            ...
            // Setup Query KIT backend for search results
            let encodedQuery = encodeURIComponent(query);
            let searchUrl = `${this.setting.KITUrl}/api/search?q=${encodedQuery}&n=${this.setting.resultsCount}&r=true&client=obsidian`;
            let headers = {
                'Authorization': `Bearer ${this.setting.KITApiKey}`,
            }

            // Get search results from KIT backend
    ...
    renderResults(results: SimilarResult[]) {
        ...
        this.resultsContainerEl.createEl("div", {
            cls: "KIT-results-count",
            text: `Found ${results.length} similar document${results.length > 1 ? 's' : ''}`
        });

        // Create results list
        const resultsListEl = this.resultsContainerEl.createEl("div", { cls: "KIT-similar-results-list" });

        // Render each result
        results.forEach(async (result) => {
            const resultEl = resultsListEl.createEl("div", { cls: "KIT-similar-result-item" });
            ...
            const headerEl = resultEl.createEl("div", { cls: "KIT-similar-result-header" });

            // Show filename with appropriate color
            const fileEl = headerEl.createEl("div", {
                cls: `KIT-result-file ${result.inVault ? 'in-vault' : 'not-in-vault'}`
            });
            ...
            const contentEl = resultEl.createEl("div", {
                cls: "KIT-result-entry KIT-similar-content-hidden"
            });
            ...
            moreContextButton.addEventListener("click", (e) => {
                ...
                if (contentEl.classList.contains("KIT-similar-content-hidden")) {
                    contentEl.classList.remove("KIT-similar-content-hidden");
                    contentEl.classList.add("KIT-similar-content-visible");
                    moreContextButton.empty();
                    moreContextButton.createSpan({ text: "Less context" });
                    setIcon(moreContextButton.createSpan(), "chevron-up");
                } else {
                    contentEl.classList.remove("KIT-similar-content-visible");
                    contentEl.classList.add("KIT-similar-content-hidden");
                    moreContextButton.empty();
                    moreContextButton.createSpan({ text: "More context" });
                    setIcon(moreContextButton.createSpan(), "chevron-down");
                }
    ...
    updateUI(state: "loading" | "no-file" | "unsupported-file" | "no-results" | "error" | "empty-query", message?: string) {
        ...
        const messageEl = this.resultsContainerEl.createEl("div", { cls: "KIT-similar-message" });
