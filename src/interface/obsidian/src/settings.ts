...
export const DEFAULT_SETTINGS: KITSetting = {
    resultsCount: 15,
    KITUrl: 'https://app.KIT.dev',
    KITApiKey: '',
...
export class KITSettingTab extends PluginSettingTab {
    ...
    constructor(app: App, plugin: KIT) {
        super(app, plugin);
        this.plugin = plugin;
    }
    ...
    display(): void {
        ...
        // Add notice whether able to connect to KIT backend or not
        let backendStatusMessage = getBackendStatusMessage(
            ...
            this.plugin.settings.KITUrl,
            this.plugin.settings.KITApiKey
        );

        const connectHeaderEl = containerEl.createEl('h3', { title: backendStatusMessage });
        const connectHeaderContentEl = connectHeaderEl.createSpan({ cls: 'KIT-connect-settings-header' });
        const connectTitleEl = connectHeaderContentEl.createSpan({ text: 'Connect' });
        const backendStatusEl = connectTitleEl.createSpan({ text: this.connectStatusIcon(), cls: 'KIT-connect-settings-header-status' });
        if (this.plugin.settings.userInfo && this.plugin.settings.connectedToBackend) {
            if (this.plugin.settings.userInfo.photo) {
                ...
                const profilePicEl = connectHeaderContentEl.createDiv({
                    text: initial,
                    cls: 'KIT-profile KIT-profile-initial'
                });
                profilePicEl.addEventListener('click', () => { new Notice(backendStatusMessage); });
            }
        }
        ...
        // Add KIT settings configurable from the plugin settings tab
        const apiKeySetting = new Setting(containerEl)
            .setName('KIT API Key')
            .addText(text => text
                .setValue(`${this.plugin.settings.KITApiKey}`)
                .onChange(async (value) => {
                    this.plugin.settings.KITApiKey = value.trim();
                    ({
                    ...
                    } = await canConnectToBackend(this.plugin.settings.KITUrl, this.plugin.settings.KITApiKey));
        ...
        apiKeySetting.descEl.createEl('span', {
            text: 'Connect your KIT Cloud account. ',
        });
        apiKeySetting.descEl.createEl('a', {
            text: 'Get your API Key',
            href: `${this.plugin.settings.KITUrl}/settings#clients`,
            attr: { target: '_blank' }
        });

        new Setting(containerEl)
            .setName('KIT URL')
            .setDesc('The URL of the KIT backend.')
            .addText(text => text
                .setValue(`${this.plugin.settings.KITUrl}`)
                .onChange(async (value) => {
                    this.plugin.settings.KITUrl = value.trim().replace(/\/$/, '');
                    ({
                        connectedToBackend: this.plugin.settings.connectedToBackend,
                        userInfo: this.plugin.settings.userInfo,
                        statusMessage: backendStatusMessage,
        ...
        new Setting(containerEl)
            .setName('Auto Sync')
            .setDesc('Automatically index your vault with KIT.')
            .addToggle(toggle => toggle
        ...
        new Setting(containerEl)
            .setName('Sync Notes')
            .setDesc('Index Markdown files in your vault with KIT.')
            .addToggle(toggle => toggle
        ...
        new Setting(containerEl)
            .setName('Sync Images')
            .setDesc('Index images in your vault with KIT.')
            .addToggle(toggle => toggle
        ...
        new Setting(containerEl)
            .setName('Sync PDFs')
            .setDesc('Index PDF files in your vault with KIT.')
            .addToggle(toggle => toggle
        ...
        indexVaultSetting
            .setName('Force Sync')
            .setDesc('Manually force KIT to re-index your Obsidian Vault.')
            .addButton(button => button
                ...
                .onClick(async () => {
                    ...
                    const syncProgressEl = document.getElementById('KIT-sync-progress') as HTMLProgressElement | null;
                    const syncProgressText = document.getElementById('KIT-sync-progress-text') as HTMLElement | null;
                    ...
                    const onProgress = (progress: { processed: number, total: number }) => {
                        const el = document.getElementById('KIT-sync-progress') as HTMLProgressElement | null;
                        const txt = document.getElementById('KIT-sync-progress-text') as HTMLElement | null;
                        if (!el || !txt) return;
    ...
    private async refreshStorageDisplay() {
        ...
        try {
            ...
            console.error('KIT: Failed to update storage display', err);
        ...
        if (this.plugin.settings.connectedToBackend) {
            ...
            if (serverConfig && serverConfig.selected_chat_model_config !== undefined && serverConfig.selected_chat_model_config !== null) {
                ...
                if (this.plugin.settings.availableChatModels.some(m => m.id === serverModelIdStr)) {
                    ...
                    // In this case, we might fall back to null (KIT Default)
                    console.warn(`KIT: Server's selected model ID ${serverModelIdStr} not in available models. Falling back to default.`);
    ...
    private renderChatModelDropdown() {
        ...
        if (!this.plugin.settings.connectedToBackend) {
            modelSetting.setDesc('Connect to KIT to load and set chat model options.');
        ...
        if (this.plugin.settings.availableChatModels.length === 0 && this.plugin.settings.connectedToBackend) {
            modelSetting.setDesc('Fetching models or no models available. Check KIT connection or try refreshing.');
