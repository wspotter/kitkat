...
def construct_chat_history_for_operator(conversation_history: List[ChatMessageModel], n: int = 6) -> list[AgentMessage]:
    ...
    Only include last n completed turns (i.e with user and KIT message).
    """
    ...
    for chat in conversation_history:
        ...
        elif chat.by == "KIT" and chat.message:
...
def message_to_log(
    ...
    KIT_message_metadata={},
    chat_history: List[ChatMessageModel] = [],
) -> List[ChatMessageModel]:
    """Create json logs from messages, metadata for conversation log"""
    default_KIT_message_metadata = {
        "intent": {"type": "remember", "memory-type": "notes", "query": user_message},
    }
    KIT_response_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    KIT_message_metadata = {k: v for k, v in KIT_message_metadata.items() if v is not None}

    # Create json log from Human's message
    human_log = merge_dicts({"message": user_message, "by": "you"}, user_message_metadata)

    # Create json log from GPT's response
    KIT_log = merge_dicts(KIT_message_metadata, default_KIT_message_metadata)
    KIT_log = merge_dicts({"message": chat_response, "by": "KIT", "created": KIT_response_time}, KIT_log)
    ...
    try:
        StrictChatMessageModel(**KIT_log)
    except ValidationError as e:
        logger.error(f"Validation error in KIT chat message: {e}\nKIT Message: {KIT_log}\n")

    human_message = ChatMessageModel(**human_log)
    KIT_message = ChatMessageModel(**KIT_log)
    chat_history.extend([human_message, KIT_message])
    return chat_history
...
async def save_to_conversation_log(
    ...
    if generated_mermaidjs_diagram:
        KIT_message_metadata["mermaidjsDiagram"] = generated_mermaidjs_diagram
    ...
    try:
        new_messages = message_to_log(
            ...
            KIT_message_metadata=KIT_message_metadata,
            chat_history=[],
        )
    except ValidationError as e:
        new_messages = None
...
def generate_chatml_messages_with_context(
    ...
    for chat in chat_history:
        ...
        if chat.by == "KIT" and chat.intent and "excalidraw" in chat.intent.type:
        ...
        # Add user query with attached file, images or KIT response
        if chat.queryFiles:
            raw_query_files = chat.queryFiles
