...
from KIT.utils import constants

if TYPE_CHECKING:
    from sentence_transformers import CrossEncoder, SentenceTransformer

    from KIT.utils.models import BaseEncoder
...
def get_server_id():
    ...
    Helps count distinct KIT servers deployed.
    Maintains anonymity by using non-PII random id."""
    # Initialize server_id to None
    server_id = None
    # Expand path to the KIT env file. It contains persistent internal app data
    ...
    if path.exists(app_env_filename):
        ...
        # Create KIT config directory if it doesn't exist
...
command_descriptions = {
    ConversationCommand.General: "Only talk about information that relies on KIT's general knowledge, not your personal knowledge base.",
...
tool_descriptions_for_llm = {
    ...
    ConversationCommand.Online: "To search for the latest, up-to-date information from the internet. Note: **Questions about KIT should always use this data source**",
...
class ImageIntentType(Enum):
    """
    Chat message intent by KIT for image responses.
...
def in_debug_mode():
    """Check if KIT is running in debug mode.
    Set KIT_DEBUG environment variable to true to enable debug mode."""
    return is_env_var_true("KIT_DEBUG")


def is_promptrace_enabled():
    """Check if KIT is running with prompt tracing enabled.
...
def is_operator_enabled():
    """Check if KIT can operate GUI applications.
    Set KIT_OPERATOR_ENABLED env var to true and install playwright to enable it."""
    return is_env_var_true("KIT_OPERATOR_ENABLED")


def is_code_sandbox_enabled():
    """Check if KIT can run code in sandbox.
    Set KIT_TERRARIUM_URL or E2B api key via env var to enable it."""
    return not is_none_or_empty(os.getenv("KIT_TERRARIUM_URL")) or is_e2b_code_sandbox_enabled()
