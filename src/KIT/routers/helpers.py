...
from KIT.database import adapters
...
from KIT.processor.content.notion.notion_to_entries import NotionToEntries
from KIT.processor.content.org_mode.org_to_entries import OrgToEntries
from KIT.processor.content.pdf.pdf_to_entries import PdfToEntries
...
from KIT.processor.conversation.anthropic.anthropic_chat import (
    anthropic_send_message_to_model,
    converse_anthropic,
)
from KIT.processor.conversation.google.gemini_chat import (
    converse_gemini,
    gemini_send_message_to_model,
)
from KIT.processor.conversation.openai.gpt import (
    converse_openai,
...
from KIT.processor.speech.text_to_speech import is_eleven_labs_enabled
from KIT.routers.email import is_resend_enabled, send_task_email
from KIT.routers.twilio import is_twilio_enabled
...
from KIT.utils.rawconfig import (
    ChatRequestBody,
    FileData,
    LocationData,
    SearchResponse,
)
from KIT.utils.state import SearchType
...
def validate_chat_model(user: KITUser):
...
async def is_ready_to_chat(user: KITUser):
    ...
    raise HTTPException(status_code=500, detail="Set your OpenAI API key or enable Local LLM via KIT settings.")
...
def update_telemetry_state(
    ...
    user: KITUser = request.user.object if request.user.is_authenticated else None
...
async def acreate_title_from_query(query: str, user: KITUser = None) -> str:
    """
...
async def acheck_if_safe_prompt(system_prompt: str, user: KITUser = None, lax: bool = False) -> Tuple[bool, str]:
    """
...
def schedule_query(
    q: str, chat_history: List[ChatMessageModel], user: KITUser, query_images: List[str] = None, tracer: dict = {}
) -> Tuple[str, str, str]:
    """
...
async def search_documents(
    ...
    if not await sync_to_async(EntryAdapters.user_has_entries)(user=user):
        if not agent_has_entries:
            logger.debug("No documents in knowledge base. Use a KIT client to sync and chat with your docs.")
            yield compiled_references, inferred_queries, q
            return
...
class EmailVerificationApiRateLimiter:
    ...
    async def __call__(self, email: str = None):
        ...
        user: KITUser = await aget_user_by_email(email)
        if not user:
...
class ApiUserRateLimiter:
    ...
    def __call__(self, request: Request):
        ...
        user: KITUser = request.user.object
        ...
        if not subscribed and count_requests >= self.requests:
            ...
            raise HTTPException(
                status_code=429,
                detail="I'm glad you're enjoying interacting with me! You've unfortunately exceeded your usage limit for today. You can subscribe to increase your usage limit via [your settings](https://app.KIT.dev/settings) or we can continue our conversation tomorrow?",
            )
    ...
    async def check_websocket(self, websocket: WebSocket):
        ...
        user: KITUser = websocket.scope["user"].object
...
class WebSocketConnectionManager:
    ...
    async def register_connection(self, user: KITUser, connection_id: str) -> None:
        """Register a new WebSocket connection."""
        await UserRequests.objects.acreate(user=user, slug=f"{self.connection_slug_prefix}{connection_id}")

    async def unregister_connection(self, user: KITUser, connection_id: str) -> None:
    ...
    async def _cleanup_stale_connections(self, user: KITUser) -> None:
...
class ConversationCommandRateLimiter:
    ...
    async def update_and_check_if_valid(self, request: Request | WebSocket, conversation_command: ConversationCommand):
        ...
        if not subscribed and count_requests >= self.trial_rate_limit:
            ...
            raise HTTPException(
                status_code=429,
                detail=f"I'm glad you're enjoying interacting with me! You've unfortunately exceeded your `/{conversation_command.value}` command usage limit for today. You can subscribe to increase your usage limit via [your settings](https://app.KIT.dev/settings) or we can talk about something else for today?",
            )
...
def format_automation_response(scheduling_request: str, executed_query: str, ai_response: str, user: KITUser) -> bool:
    """
...
def should_notify(original_query: str, executed_query: str, ai_response: str, user: KITUser) -> bool:
    """
...
def scheduled_chat(
    ...
    url = f"{scheme}://{parsed_url.netloc}/api/chat?client=KIT"

    # Construct the Headers for the chat API
    headers = {"User-Agent": "KIT", "Content-Type": "application/json"}
    if not state.anonymous_mode:
        # Add authorization request header in non-anonymous mode
        token = get_KIT_tokens(user)
        if is_none_or_empty(token):
            token = create_KIT_token(user).token
        else:
            token = token[0].token
...
def get_user_config(user: KITUser, request: Request, is_detailed: bool = False):
    ...
    return {
        ...
        # server settings
        "KIT_cloud_subscription_url": os.getenv("KIT_CLOUD_SUBSCRIPTION_URL"),
...
def get_notion_auth_url(user: KITUser):
