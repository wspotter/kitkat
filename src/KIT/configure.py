...
from KIT.database.models import ClientApplication, KITUser, ProcessLock, Subscription
...
from KIT.utils import constants, state
from KIT.utils.config import SearchType
from KIT.utils.helpers import is_none_or_empty

logger = logging.getLogger(__name__)


class AuthenticatedKITUser(SimpleUser):
...
class UserAuthenticationBackend(AuthenticationBackend):
    ...
    def _initialize_default_user(self):
        if not self.KITuser_manager.filter(username="default").exists():
            default_user = self.KITuser_manager.create_user(
                username="default",
    ...
    async def authenticate(self, request: HTTPConnection):
        ...
        if current_user and current_user.get("email"):
            try:
                user = (
                    await self.KITuser_manager.filter(email=current_user.get("email"))
            ...
            if user:
                ...
                return AuthCredentials(["authenticated"]), AuthenticatedKITUser(user)
        ...
        if len(request.headers.get("Authorization", "").split("Bearer ")) == 2:
            ...
            try:
                user_with_token = (
                    await self.KITapiuser_manager.filter(token=bearer_token)
                    .select_related("user")
            ...
            if user_with_token:
                ...
                if subscribed:
                    return AuthCredentials(["authenticated", "premium"]), AuthenticatedKITUser(user_with_token.user)
                return AuthCredentials(["authenticated"]), AuthenticatedKITUser(user_with_token.user)
        ...
        if client_id:
            ...
            if subscribed:
                return AuthCredentials(["authenticated", "premium"]), AuthenticatedKITUser(user, client_application)
            return AuthCredentials(["authenticated"]), AuthenticatedKITUser(user, client_application)

        # No auth required if server in anonymous mode
        if state.anonymous_mode:
            try:
                user = await self.KITuser_manager.filter(username="default").prefetch_related("subscription").afirst()
            except (DatabaseError, OperationalError):
...
def initialize_content(user: KITUser, regenerate: bool, search_type: Optional[SearchType] = None):
...
def configure_routes(app):
    ...
    from KIT.routers.api import api
    from KIT.routers.api_agents import api_agents
    from KIT.routers.api_automation import api_automation
    from KIT.routers.api_chat import api_chat
    from KIT.routers.api_content import api_content
    from KIT.routers.api_memories import api_memories
    from KIT.routers.api_model import api_model
    from KIT.routers.notion import notion_router
    from KIT.routers.web_client import web_client
    ...
    if not state.anonymous_mode:
        from KIT.routers.auth import auth_router

        app.include_router(auth_router, prefix="/auth")
        logger.info("🔑 Enabled Authentication")

    if state.billing_enabled:
        from KIT.routers.api_subscription import subscription_router

        app.include_router(subscription_router, prefix="/api/subscription")
        logger.info("💳 Enabled Billing")

    if is_twilio_enabled():
        from KIT.routers.api_phone import api_phone
...
def configure_middleware(app, ssl_enabled: bool = False):
    ...
    app.add_middleware(SessionMiddleware, secret_key=os.environ.get("KIT_DJANGO_SECRET_KEY", "!secret"))
