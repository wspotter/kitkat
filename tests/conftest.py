...
@pytest.fixture
def default_user2():
    if KITUser.objects.filter(username="default").exists():
        return KITUser.objects.get(username="default")

    user = KITUser.objects.create(
        username="default",
...
def default_user3():
    ...
    """
    if KITUser.objects.filter(username="default3").exists():
        return KITUser.objects.get(username="default3")
...
def default_user4():
    ...
    """
    if KITUser.objects.filter(username="default4").exists():
        return KITUser.objects.get(username="default4")
...
def api_user(default_user):
    if KITApiUser.objects.filter(user=default_user).exists():
        return KITApiUser.objects.get(user=default_user)

    return KITApiUser.objects.create(
        user=default_user,
        name="api-key",
        token="kk-secret",
    )
...
def api_user2(default_user2):
    if KITApiUser.objects.filter(user=default_user2).exists():
        return KITApiUser.objects.get(user=default_user2)
...
def api_user3(default_user3):
    if KITApiUser.objects.filter(user=default_user3).exists():
        return KITApiUser.objects.get(user=default_user3)
...
def api_user4(default_user4):
    if KITApiUser.objects.filter(user=default_user4).exists():
        return KITApiUser.objects.get(user=default_user4)
...
def chat_client(search_config, default_user2: KITUser):
    return chat_client_builder(search_config, default_user2, require_auth=False)


@pytest.fixture(scope="function")
def chat_client_with_auth(search_config, default_user2: KITUser):
    return chat_client_builder(search_config, default_user2, require_auth=True)


@pytest.fixture(scope="function")
def chat_client_no_background(search_config, default_user2: KITUser):
    return chat_client_builder(search_config, default_user2, index_content=False, require_auth=False)


@pytest.fixture(scope="function")
def chat_client_with_large_kb(search_config, default_user2: KITUser):
    """
...
def large_kb_chat_client_builder(search_config, user):
    ...
    temp_dir = tempfile.mkdtemp(prefix="KIT_test_large_kb_")
    file_type = "markdown"
    large_file_list = []
...
@pytest.fixture(scope="function")
def pdf_configured_user1(default_user: KITUser):
