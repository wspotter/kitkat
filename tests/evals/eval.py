...
# Configuration
KIT_URL = os.getenv("KIT_URL", "http://localhost:42110")
KIT_CHAT_API_URL = f"{KIT_URL}/api/chat"
KIT_API_KEY = os.getenv("KIT_API_KEY")
KIT_MODE = os.getenv("KIT_MODE", "default").lower()  # E.g research, general, default etc.

GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")
GEMINI_EVAL_MODEL = os.getenv("GEMINI_EVAL_MODEL", "gemini-2.5-flash")

LLM_SEED = int(os.getenv("KIT_LLM_SEED")) if os.getenv("KIT_LLM_SEED") else None
...
SLEEP_SECONDS = 3 if KIT_MODE == "general" else 1  # Sleep between API calls to avoid rate limiting
KIT_API_TIMEOUT_SECONDS = 1200  # Default to 20 minutes
...
def index_frames_kb():
    """Index Wikipedia articles from FRAMES dataset into KIT"""
    try:
        # Load dataset
        dataset = load_frames_kb()
        dataset_files = set(map(get_article_filename, dataset))

        # Get indexed files from KIT API
        headers = {"Authorization": f"Bearer {KIT_API_KEY}"} if KIT_API_KEY else {}
        try:
            response = requests.get(f"{KIT_URL}/api/content/computer", headers=headers)
        ...
        for batch in tqdm(batcher(filtered_dataset, batch_size), total=total_batches, desc="Indexing FRAMES KB"):
            ...
            try:
                response = requests.patch(f"{KIT_URL}/api/content?client=eval", headers=headers, files=files)
                response.raise_for_status()
...
def get_agent_response(prompt: str) -> Dict[str, Any]:
    """Get response from the KIT API"""
    # Set headers
    headers = {"Content-Type": "application/json"}
    if not is_none_or_empty(KIT_API_KEY):
    ...
    try:
        response = requests.post(
            ...
            timeout=KIT_API_TIMEOUT_SECONDS,
        )
        response.raise_for_status()
...
def evaluate_response_for_ir(
    ...
    """Evaluate KIT response against benchmark ground truth using string matching"""
    try:
...
def evaluate_response_with_gemini(
    ...
    """Evaluate KIT response against benchmark ground truth using Gemini"""
    evaluation_prompt = f"""
...
def process_batch(batch, batch_start, results, dataset_length, response_evaluator):
    ...
    for idx, (prompt, answer, reasoning_type) in enumerate(batch):
        ...
        prompt = f"/{KIT_MODE} {prompt}" if KIT_MODE and not prompt.startswith(f"/{KIT_MODE}") else prompt
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate KIT on a supported benchmark.")
    parser.add_argument(
        "--output",
        "-o",
        default=None,
...
if __name__ == "__main__":
    """
    Evaluate KIT on supported benchmarks.
    Response are evaluated by GEMINI_EVAL_MODEL (default: gemini-2.5-flash).

    KIT should be running at KIT_URL (default: http://localhost:42110).
    The Gemini judge model is accessed via the Gemini API with your GEMINI_API_KEY.
    To evaluate KIT in research mode, set the KIT_MODE environment variable to "research".
